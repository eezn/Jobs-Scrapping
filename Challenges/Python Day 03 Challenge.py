...
def update_word(my_dict, key=None, val=None):
    if type(my_dict) is not dict:
        print("You need to send a dictionary. You sent: {0}".format(
            type(my_dict)))
    elif key == None or val == None:
...
def delete_from_dict(my_dict, key=None):
    if type(my_dict) is not dict:
        ...
        print("You need to soecify a word to delete.")
    else:
        if my_dict.get(key) != None:
            my_dict.pop(key)
